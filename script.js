...
function addToCart(item, type) {
  const cartItem = {
    ...
    price:
      type === "bundle"
        ? item.bundle_price
        : item.discount_price || item.sell_price,
...
function updateCartDisplay() {
  ...
  if (cart.length === 0) {
    cartItems.innerHTML =
      '<div class="empty-cart"><p>Your cart is empty</p><p>Add some products to get started!</p></div>';
